# Dokploy Deployment Guide for sbali.in
...
- **Name**: sbali-frontend
- **Type**: Docker/Next.js
- **Domain**: sbali.in (and www.sbali.in)
- **Port**: 3000
...
- **Access**: Internal only (localhost) or minio.sbali.in for admin
...
```
MONGO_URI=mongodb://sbali_admin:SECURE_PASSWORD@localhost:27017/sbali_production
```
...
```nginx
# /etc/nginx/sites-available/sbali.in
...
# Get certificates
sudo certbot --nginx -d sbali.in -d www.sbali.in
sudo certbot --nginx -d api.sbali.in
...
1. **Connect GitHub Repository**
   ...
   - Select Deoshabh/Sbali-2026 repository
   - Set branch to `main`
...
dokploy update sbali-frontend

# Backup MongoDB
mongodump --uri="mongodb://localhost:27017/sbali_production" --out=/backups/$(date +%Y%m%d)

...
curl -X POST https://api.sbali.in/api/v1/admin/products \
  -H "Content-Type: application/json" \
  ...
  -d '{
    ...
    "stock": 100,
    "images": [{
      "url": "https://minio.sbali.in/product-media/test/test.png",
      "key": "test/test.png",
      "isPrimary": true,
      "order": 0
    }],
...
1. Open your website: `https://sbali.in`
...
```javascript
const allowedOrigins = ["https://sbali.in", "https://www.sbali.in"];
```
...
- [ ] API endpoint is accessible: `https://api.sbali.in/api/v1/products`
- [ ] Frontend `NEXT_PUBLIC_API_URL` is set to: `https://api.sbali.in/api/v1`

...
**Backend API (api.sbali.in)**

```yaml
traefik.http.routers.backend.rule=Host(`api.sbali.in`)
...
**From your browser console (when on sbali.in):**
...
4. Verify backend service is healthy: `curl https://api.sbali.in/health`
...
1. **CORS Origin Restriction**: Only `https://sbali.in` and `https://www.sbali.in` are allowed
